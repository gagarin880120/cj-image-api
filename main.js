...
function changeMatrix(n) {
  matrixSize = n;
  localStorage.setItem('matrix', matrixSize);
...
function startup() {
  ...
  colorInputEl.addEventListener('input', function (event) {
...
document.addEventListener('click', function (event) {
  if (window.state.currentTool === 'fillBucket') {
...
pencilBtn.addEventListener('click', function () {
  window.state.currentTool = 'pencilTool';
});
var painting = false;
var lastX = 0;
var lastY = 0;
...
function draw(e) {
  ...
  for (var _x2 = x1; _x2 < x2; _x2 += 1) {
    if (steep) {
      ctx.fillRect(Math.floor(y / (512 / matrixSize)), Math.floor(_x2 / (512 / matrixSize)), lineThickness, lineThickness);
    } else {
      ctx.fillRect(Math.floor(_x2 / (512 / matrixSize)), Math.floor(y / (512 / matrixSize)), lineThickness, lineThickness);
    }
...
function pencilTool() {
  if (window.state.currentTool === 'pencilTool') {
...
document.addEventListener('click', pencilTool); // =================================LOAD-IMAGES==========================

var x128 = document.getElementById('128x128');
var x256 = document.getElementById('256x256');
var x512 = document.getElementById('512x512');

function drawImgResized() {
  ...
  img.onload = function () {
    ...
    if (img.width > img.height) {
      var imageHeight = img.height * (matrixSize / img.width);
      var imageWidth = matrixSize;
      var startY = (matrixSize - imageHeight) / 2;
      ctx.drawImage(img, 0, startY, imageWidth, imageHeight);
    } else if (img.width < img.height) {
      var _imageHeight = matrixSize;

      var _imageWidth = img.width * (matrixSize / img.height);

      var startX = (matrixSize - _imageWidth) / 2;
      ctx.drawImage(img, startX, 0, _imageWidth, _imageHeight);
    } else {
...
x128.addEventListener('click', function () {
  changeMatrix(128);
  drawImgResized();
});
x256.addEventListener('click', function () {
  changeMatrix(256);
  drawImgResized();
});
x512.addEventListener('click', function () {
  changeMatrix(512);
  drawImgResized();
});

function drawImg(link) {
  ctx.clearRect(0, 0, 512, 512);
  var img = new Image();
  ...
  img.onload = function () {
    ...
    if (img.width > img.height) {
      ...
      var _imageHeight2 = matrixSize;

      var _imageWidth2 = img.width * (matrixSize / img.height);

      var startX = (matrixSize - _imageWidth2) / 2;
      ctx.drawImage(img, startX, 0, _imageWidth2, _imageHeight2);
    } else {
      ctx.drawImage(img, 0, 0, matrixSize, matrixSize);
    }
...
function getLinkToImage() {
  return _getLinkToImage.apply(this, arguments);
}

function _getLinkToImage() {
  _getLinkToImage = _babel_runtime_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_1___default()(
  /*#__PURE__*/
    ...
    return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            url = 'https://api.unsplash.com/photos/random?query=town,Minsk&client_id=26e0be6da3beaec44edf52aa1593502e2c7ab9d6ce71a1c59a9c9d9a4541324a';

            if (searchText.value) {
              url = "https://api.unsplash.com/photos/random?query=town,".concat(searchText.value, "&client_id=26e0be6da3beaec44edf52aa1593502e2c7ab9d6ce71a1c59a9c9d9a4541324a");
            }
...
window.onload = function () {
  ...
  if (localStorage.getItem('matrix') && localStorage.getItem('dataURL') && localStorage.getItem('currColor')) {
    matrixSize = +localStorage.getItem('matrix');
    currentColor = localStorage.getItem('currColor');
    updateCurr();
    prevColorEl.style.background = localStorage.getItem('prevColor');
    startup();
    dataURL = localStorage.getItem('dataURL');
    drawImg(dataURL);
    pencilTool();
  } else if (localStorage.getItem('dataURL') && localStorage.getItem('currColor')) {
    currentColor = localStorage.getItem('currColor');
    updateCurr();
    prevColorEl.style.background = localStorage.getItem('prevColor');
    startup();
    ...
    drawImg(dataURL);
    pencilTool();
  } else if (localStorage.getItem('dataURL') && localStorage.getItem('matrix')) {
    matrixSize = +localStorage.getItem('matrix');
    updateCurr();
    startup();
    dataURL = localStorage.getItem('dataURL');
